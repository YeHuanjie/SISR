...
class RDB(torch.nn.Module):
    def __init__(self, in_channels, growth_rate, num_layers, block):
...
class RRDB(torch.nn.Module):
    ...
    def __init__(self, scale_factor, num_channels, num_features, growth_rate, num_blocks, num_layers):
        ...
        # in G0+G+G out G
        # ......
        self.rdbs = torch.nn.ModuleList([RDB(self.G0, self.G, self.C, 0)])
        for i in range(self.D - 1):
            self.rdbs.append(RDB(self.G0 * (2 ** (i + 1)), self.G, self.C, i + 1))

        # part3: dense feature fusion
        self.gff = torch.nn.Sequential(
            torch.nn.Conv2d(self.G0 * (2 ** self.D), self.G0, kernel_size=1),
            # 1x1 conv
...
class RRDBTrainer(object):
    def __init__(self, flags, train_loader, test_loader):
        super(RRDBTrainer, self).__init__()
        self.model = None
    ...
    def bulid_model(self):
        ...
        # self.model = torch.nn.DataParallel(RRDB(self.upscale, 3, self.num_features,
        #                                        self.growth_rate, self.num_blocks, self.num_layers)).cuda()
        self.model = RRDB(self.upscale, 3, self.num_features, self.growth_rate, self.num_blocks, self.num_layers).cuda()
