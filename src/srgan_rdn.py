...
#     def __init__(self, in_channels=3, upscale=4, num_feature=64, num_layer=5):
...
#
#         # part3: fusion
#         self.fusion = torch.nn.Conv2d(num_feature, num_feature, kernel_size=1, padding=0)
#         # 1x1
#
#         # part4: upsample
...
#
#         # part5: out
#         self.out = torch.nn.Conv2d(num_feature, in_channels, kernel_size=3, padding=3 // 2)
#
...
class Generator(torch.nn.Module):
    def __init__(self, upscale, in_channels, num_features, growth_rate, num_blocks, num_layers):
        # RDN + tanh
        super(Generator, self).__init__()
        self.rdn = rdn_4gan.RDN(upscale, in_channels, num_features, growth_rate, num_blocks, num_layers)
        self.tanh = torch.nn.Tanh()
...
class Discriminator(torch.nn.Module):
    def __init__(self, in_channels=3, num_feature=64):
        ...
        self.net = torch.nn.Sequential(
            ...
            # torch.nn.Conv2d(8 * num_feature, 16 * num_feature, kernel_size=3, padding=1),
            # torch.nn.BatchNorm2d(16 * num_feature),
            # torch.nn.LeakyReLU(0.2),
            #
            # torch.nn.Conv2d(16 * num_feature, 16 * num_feature, kernel_size=3, stride=2, padding=1),
            # torch.nn.BatchNorm2d(16 * num_feature),
            # torch.nn.LeakyReLU(0.2),

            torch.nn.AdaptiveAvgPool2d(1),
            torch.nn.Conv2d(8 * num_feature, 18 * num_feature, kernel_size=1),
            torch.nn.LeakyReLU(0.2),
            torch.nn.Conv2d(18 * num_feature, 1, kernel_size=1)
        )
...
class SRGANTrainer(object):
    def __init__(self, flags, train_loader, test_loader):
        ...
        # RRDB
        # self.num_features = flags.rrdb_num_features
        # self.growth_rate = flags.rrdb_growth_rate
        # self.num_blocks = flags.rrdb_num_blocks
        # self.num_layers = flags.rrdb_num_layers
    ...
    def bulid_model(self):
        ...
        # self.G = torch.nn.DataParallel(...).cuda()
        # self.D = torch.nn.DataParallel(...).cuda()

        # RDN
        # self.G = rdn.RDN(self.upscale, 3, self.num_features,
        #                  self.growth_rate, self.num_blocks, self.num_layers).cuda()
        self.G = \
            Generator(self.upscale, 3, self.num_features, self.growth_rate, self.num_blocks, self.num_layers).cuda()

        # RRDB
        # self.G = rrdb.RRDB(self.upscale, 3, self.num_features,
        #                    self.growth_rate, self.num_blocks, self.num_layers).cuda()
    ...
    def train(self):
        ...
        f = open(f_path, 'a+')
        print("this epoch G_loss: %.6f, D_loss: %.6f, D(x): %.6f, D(G(X)): %.6f"
              % (train_g_loss, train_d_loss, train_d_score, train_g_score), file=f)
        f.close()
        # return train_g_loss, train_d_loss, train_g_score, train_d_score
